  ...
  assert.deepEqual(
    {
      ...migrated
        .prepare(
          `SELECT entry_id, entry_type, text FROM session_search
           WHERE session_id = 'session-2' AND entry_type = 'session_title'`
        )
        .get(),
    },
    { entry_id: "", entry_type: "session_title", text: "title" }
  )
  assert.equal(
    migrated
      .prepare("SELECT count(*) AS count FROM project_registrations")
...
test("database v10 migration rebuilds existing search rows with trigram indexing", async () => {
  const root = await mkdtemp(path.join(tmpdir(), "pi-web-codex-db-v10-"))
  const previousConfigDir = process.env.PI_WEB_CODEX_CONFIG_DIR
  process.env.PI_WEB_CODEX_CONFIG_DIR = root
  const databasePath = path.join(root, "state.db")
  const legacy = new DatabaseSync(databasePath)
  legacy.exec(`
    ...
    INSERT INTO session_search VALUES (
      'session-existing', 'entry-existing', 'message',
      '2026-07-14T00:00:00.000Z', '请务必实际调用 bash 工具'
    );
  ...
  assert.deepEqual(
    {
      ...migrated
        .prepare(
          `SELECT session_id, entry_id, text
           FROM session_search
           WHERE session_search MATCH '"实际调用"'`
        )
        .get(),
    },
    {
      ...
      text: "请务必实际调用 bash 工具",
    }
  )
  assert.match(
    String(
      migrated
        .prepare("SELECT sql FROM sqlite_schema WHERE name = 'session_search'")
        .get()?.sql
    ),
    /tokenize = 'trigram'/u
  )
...
test("database v12 migration creates session file probes", async () => {
  ...
  const orderTable = migrated
    .prepare(
      "SELECT name, type FROM sqlite_schema WHERE name = 'workspace_nav_order'"
    )
    .get() as { name: string; type: string } | undefined
  assert.equal(orderTable?.name, "workspace_nav_order")
  assert.equal(orderTable?.type, "table")
  migrated.close()
  globalThis.piWebCodexDatabase = undefined
  await rm(root, { recursive: true, force: true })
  if (previousConfigDir === undefined)
...
test("database v13 migration creates sidebar ordering storage", async () => {
  const root = await mkdtemp(path.join(tmpdir(), "pi-web-codex-db-v13-"))
  const previousConfigDir = process.env.PI_WEB_CODEX_CONFIG_DIR
  process.env.PI_WEB_CODEX_CONFIG_DIR = root
  const databasePath = path.join(root, "state.db")
  const legacy = new DatabaseSync(databasePath)
    ...
    migrated
      .prepare(
        `SELECT name FROM sqlite_schema
         WHERE type = 'table' AND name = 'workspace_nav_order'`
      )
      .get()?.name,
    "workspace_nav_order"
  )
