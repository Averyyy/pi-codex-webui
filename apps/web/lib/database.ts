...
async function openDatabase() {
  ...
  if (version === 0) {
    database.exec(`
      ...
      CREATE TABLE workspace_nav_order (
      ...
      CREATE TABLE sessions (
        ...
        archived_at TEXT,
        pinned_at TEXT,
        completion_unread INTEGER NOT NULL DEFAULT 0
          CHECK (completion_unread IN (0, 1)),
      ...
      CREATE TABLE session_file_probes (
        file TEXT PRIMARY KEY,
        file_mtime_ns TEXT NOT NULL,
        size INTEGER NOT NULL,
        kind TEXT NOT NULL CHECK (kind IN ('session', 'foreign')),
        cwd TEXT
      ) STRICT;
